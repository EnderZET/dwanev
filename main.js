...
async function syncProcess() {
        ...
        await updateFileContent(
                "backups/laporan.json",
                data.laporanTable,
                `Add laporan backup ${formattedDate}`,
        );
        await updateFileContent(
                "backups/controldata.json",
                data.controlData,
                `Add control backup ${formattedDate}`,
        );
        await updateFileContent(
                "backups/skor.json",
                data.skorDb,
                `Add skor backup ${formattedDate}`,
        );
}
...
app.use(async (req, res, next) => {
        ...
        await axios
                .get(`https://api.github.com/gists/${process.env.GIST_ID}`, {
